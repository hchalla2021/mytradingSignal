...
const EarlyWarningCard: React.FC<EarlyWarningCardProps> = ({ symbol }) => {
  ...
  // ✅ REST API POLLING - Reliable and Fast
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        console.error(`[EarlyWarning] Error fetching ${symbol}:`, err);
    ...
    <div className="bg-gradient-to-br from-gray-900 via-emerald-950/10 to-gray-900 rounded-xl p-6 border-2 border-emerald-500/40 hover:border-emerald-400/60 transition-all shadow-xl shadow-emerald-500/10 hover:shadow-emerald-500/20">
      ...
      <div className={`rounded-xl p-5 mb-4 border-2 backdrop-blur-sm border-emerald-400/25 ${getSignalBg()}`}>
        <div className="flex items-center justify-between">
          <div>
            <div className="text-xs text-gray-400 mb-1">PREDICTIVE SIGNAL</div>
        ...
        <div className="grid grid-cols-2 gap-4 mt-4 pt-4 border-t border-emerald-400/15">
          <div>
            ...
            <div className="flex items-center gap-2">
              <div className="flex-1 bg-gray-800/30 rounded-full h-2.5 border border-emerald-400/20">
                <div 
                  className="bg-gradient-to-r from-emerald-500/80 to-green-400/70 h-2.5 rounded-full transition-all"
                  style={{ width: `${data.confidence ?? 0}%` }}
                />
              </div>
              <span className="text-sm font-bold text-emerald-400">{data.confidence ?? 0}%</span>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              <div className="flex-1 bg-gray-800/30 rounded-full h-2.5 border border-emerald-400/20">
                <div 
                  className="bg-gradient-to-r from-cyan-500/80 to-blue-400/70 h-2.5 rounded-full transition-all"
                  style={{ width: `${data.strength ?? 0}%` }}
                />
              </div>
              <span className="text-sm font-bold text-cyan-400">{data.strength ?? 0}%</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-gray-800/20 rounded-lg p-3 mb-3 border border-emerald-400/25 hover:border-emerald-400/35 transition-all">
        ...
        <div className="grid grid-cols-3 gap-2 text-xs">
          ...
          <div>
            ...
            <div className="text-white font-semibold">{(data.momentum?.acceleration ?? 0).toFixed(2)}%</div>
          </div>
          <div>
            <div className="text-gray-500">Consistency</div>
            <div className="text-white font-semibold">{data.momentum?.consistency ?? 0}%</div>
          </div>
        </div>
      </div>

      {/* Volume Buildup Progress */}
      <div className="bg-gray-800/20 rounded-lg p-3 mb-3 border border-emerald-400/25 hover:border-emerald-400/35 transition-all">
        <div className="flex items-center gap-2 mb-2">
          <Volume2 className="w-4 h-4 text-emerald-400" />
          <span className="text-xs font-semibold text-emerald-300">VOLUME BUILDUP</span>
          {data.volume_buildup?.is_building && (
            <span className="ml-auto text-xs text-emerald-400 font-bold animate-pulse">● ACTIVE</span>
          )}
        </div>
        <div className="flex items-center gap-2">
          <div className="flex-1 bg-gray-900/50 rounded-full h-2.5 border border-emerald-400/20">
            <div 
              className={`h-2.5 rounded-full transition-all ${
                data.volume_buildup?.is_building ? 'bg-gradient-to-r from-emerald-500/80 to-green-400/70' : 'bg-gray-700/60'
              }`}
      ...
      <div className="bg-gray-800/20 rounded-lg p-3 mb-3 border border-emerald-400/25 hover:border-emerald-400/35 transition-all">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-xs font-semibold text-emerald-300">PRICE COMPRESSION</span>
          {data.price_compression?.is_compressed && (
            <span className="ml-auto text-xs text-emerald-400 font-bold animate-pulse">● DETECTED</span>
          )}
        </div>
        <div className="flex items-center gap-2">
          <div className="flex-1 bg-gray-900/50 rounded-full h-2.5 border border-emerald-400/20">
            <div 
              className={`h-2.5 rounded-full transition-all ${
                data.price_compression?.is_compressed ? 'bg-gradient-to-r from-cyan-500/80 to-emerald-400/70' : 'bg-gray-700/60'
              }`}
      ...
      {data.fake_signal_checks && (
        <div className="bg-gray-800/20 rounded-lg p-3 mb-4 border border-emerald-400/25 hover:border-emerald-400/35 transition-all">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs font-semibold text-emerald-300">SIGNAL VALIDATION</span>
            <span className="ml-auto text-xs text-white font-bold bg-emerald-500/15 px-2 py-0.5 rounded border border-emerald-400/30">
