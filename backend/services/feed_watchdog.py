...
if __name__ == "__main__":
    ...
    async def test():
        ...
        async def mock_reconnect():
            print("   → Reconnect callback triggered!")
        
        async def mock_state_change(old, new):
            print(f"   → State changed: {old.value} → {new.value}")
        
        watchdog = FeedWatchdog(stale_threshold_seconds=5)
        await watchdog.start(
            on_reconnect=mock_reconnect,
            on_state_change=mock_state_change
        )
        ...
        # Simulate ticks
        for i in range(5):
            watchdog.on_tick(f"NIFTY")
            await asyncio.sleep(random.uniform(0.5, 2))
        
        # Simulate stale feed (no ticks for 6 seconds)
        print("\n⏸️ Simulating stale feed (no ticks for 7 seconds)...")
        ...
        # Get metrics
        metrics = watchdog.get_health_metrics()
        print(f"\nFinal Metrics:")
        ...
        print(f"  Quality: {metrics['connection_quality']:.1f}%")
